...
import configureMulter from '../config/multerConfig'; // Import the common Multer configuration function
...
const uploadCategory = configureMulter('categories');
 ...
 */
router.route('/')
  .post(uploadCategory.single('categoryImage'), createCategory) // Use the common Multer instance
  .get(getCategories);
...
router.route('/:id')
  .get(getCategoryById)
  .put(uploadCategory.single('categoryImage'), updateCategory) // Use the common Multer instance
