 ...
 */
router.route('/')
  .post(upload.single('image'), createClass)
  .get(getClasses);
 ...
 * message: { type: string, example: "Server Error" }
 */
router.post('/filter', getFilteredClasses);
 ...
 */
router.get('/live', getLiveClassByCourseId);
 ...
 * example: "Server Error"
 */
router.get('/flags', getClassesByFlags);

router.route('/:id')
  .get(getClassById)
  .put(upload.single('image'), updateClass)
  .delete(deleteClass);
 ...
 * message: { type: string, example: "Server Error" }
 */
router.patch('/:id/toggle-status', toggleClassStatus);
 ...
 */
router.get('/:classId/recordings', getClassRecordings);
