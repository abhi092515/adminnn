...
import Course, { ICourse, IDemoVideo, IFAQItem, IFacultyDetails } from '../models/Course';
...
import {
  createCourseSchema,
  updateCourseSchema,
  getCourseByIdBodySchema,
  courseSearchSchema,
  courseFilterSchema,
  courseFilterBodySchema,
  assignClassesSchema,
  unassignClassesSchema
...
export const updateCourse = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const updatesFromBody = updateCourseSchema.parse(req.body);
...
export const getCourses = async (req: Request, res: Response): Promise<void> => {
  try {
    const coursesAggregation = await Course.aggregate([
      {
        $lookup: {
          from: 'maincategories',
...
export const deleteCourse = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (courseItem) {
      if (courseItem.banner) {
        const s3Key = extractKeyFromS3Url(courseItem.banner);
        if (s3Key) {
          try {
      ...
      await Course.deleteOne({ _id: courseItem._id });
...
// @access  Public
export const getCourseById2 = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const searchCourses = async (req: Request, res: Response): Promise<void> => {
  try {
    const validationResult = courseSearchSchema.safeParse(req.query);
...
// @access  Public
export const getClassesByCourse = async (req: Request, res: Response): Promise<void> => {
  try {
    const { courseId } = req.params;
    const { populate = 'basic' } = req.query as { populate?: string };
      ...
      { $match: { _id: new Types.ObjectId(courseId) } },
      {
        $lookup: {
          ...
          pipeline: [
            ...
            {
              $project: {
                _id: 0, id: '$_id', title: 1, description: 1, duration: 1, status: 1, createdAt: 1, isFree: 1, link: 1, image: 1,
                ...(populate === 'full' && { teacherName: 1, section: 1, topic: 1 })
    ...
    courseData.classes.forEach((classItem: any) => {
      const topicName = classItem.topic?.topicName || 'No Topic';
...
export const getCourseWithUserProgress = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const assignClassesToCourses = async (req: Request, res: Response): Promise<void> => {
  try {
    const validationResult = assignClassesSchema.safeParse(req.body);
    if (!validationResult.success) {
    ...
    const coursesFound = await Course.countDocuments({ '_id': { $in: courseIds } });
    if (coursesFound !== courseIds.length) {
      res.status(404).json({ state: 404, message: 'One or more courses not found.', data: null });
      return;
    }
    const classesFound = await Class.countDocuments({ '_id': { $in: classIds } });
    ...
    const updatedCourses = await Course.find({ '_id': { $in: courseIds } }).populate('classes').exec();
...
// @access  Public
export const unassignClassesFromCourses = async (req: Request, res: Response): Promise<void> => {
  try {
    const validationResult = unassignClassesSchema.safeParse(req.body);
    if (!validationResult.success) {
      res.status(400).json({ state: 400, message: 'Validation failed', data: validationResult.error.errors });
      return;
    }
...
// @access  Public
export const postFilteredCourses = async (req: Request, res: Response): Promise<void> => {
  try {
    const validationResult = courseFilterBodySchema.safeParse(req.body);
    if (!validationResult.success) {
      res.status(400).json({
        state: 400,
    ...
    if (isRecorded !== undefined) query.isRecorded = isRecorded;
